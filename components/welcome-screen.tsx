                ...
                <div className="relative max-w-7xl mx-auto px-6 py-16 text-center">
                    ...
                    <div className="mt-8 animate-in zoom-in-95 duration-500">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-white text-[10px] font-black uppercase tracking-[0.2em] mb-6">
                            ...
                            {isCasosMode ? 'Análisis de Segmento' : 'Protocolo de Auditoría Activo'}
                        </div>

                        <h1 className="text-4xl md:text-5xl font-black tracking-tighter leading-none mb-6">
                            {isCasosMode ? (
                                <>Casos Testigo: <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-200">{segmento}</span></>
                            ) : (
                        ...
                        <p className="max-w-2xl mx-auto text-lg text-slate-300 font-medium leading-relaxed">
                            {isCasosMode
                                ...
                                : <>Bienvenido al <span className="text-white font-bold">Motor de Detección de Riesgos</span>. Visualiza clientes activos en tiempo real.</>
                            }
                        </p>
                    </div>
                    ...
                    {!isCasosMode && (
                        <div className="mt-8">
                            <div className="inline-block px-6 py-3 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm">
                                <div className="text-3xl font-black text-white leading-none">{clientes.length}</div>
                                <div className="text-[10px] text-slate-400 uppercase tracking-wider font-bold mt-1">Clientes</div>
                            </div>
                        </div>
                    )}
                </div>
                ...
                {isCasosMode ? (
                    ...
                    <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                <h2 className="text-2xl font-black text-slate-900">Clientes Registrados</h2>
                                <p className="text-sm text-slate-500 mt-1">Selecciona un cliente para ver sus casos testigo.</p>
                            </div>
                        </div>
                        <ClientesGrid
                            clientes={clientes}
                            loading={loading}
                            getHref={(cliente) => `/demo?segmento=${cliente.segmento}`}
                        />
                    </section>
                )}
            </main>
